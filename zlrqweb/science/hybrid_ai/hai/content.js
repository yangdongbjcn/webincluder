...
function getOption() {
	...
	for(var i = 0; i < g_var.q_time_line.length; i ++) {
		...
		if(g_var.q_map_scatter1[time_index]){
			var scatter_data = YdMapOps().ScatterCoord(g_var.q_map_scatter1[time_index]);
	        var scatter_serie = new YdScatterSize(scatter_data, f_symbol_size, 'rect', [-25,30], '#6600CC').name('基础理论').get();
			t_map_series[time_index].push(scatter_serie);
		}
		if(g_var.q_map_scatter2[time_index]){
			var scatter_data = YdMapOps().ScatterCoord(g_var.q_map_scatter2[time_index]);
	        var scatter_serie = new YdScatterSize(scatter_data, f_symbol_size, 'diamond', [0,30], '#33FF33').name('共性关键技术').get();
			t_map_series[time_index].push(scatter_serie);
		}
	    if(g_var.q_map_scatter3[time_index]){
	        var scatter_data = YdMapOps().ScatterCoord(g_var.q_map_scatter3[time_index]);
	        var scatter_serie = new YdScatterSize(scatter_data, f_symbol_size, 'circle', [25,30], '#ddb926').name('支撑平台').get();
	        t_map_series[time_index].push(scatter_serie);
	    }
...
$(document).ready(function(){
	p_url = g_var.gs_yd_chart + '/ChartData/apiEmpty';
	p_data = {
	};
